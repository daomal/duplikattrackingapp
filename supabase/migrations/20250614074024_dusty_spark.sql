/*
  ...
  1. Database Schema
    ...
    - Enhance shipments table with GPS tracking and driver relations
    - Add status_history table for audit trail
    - Add notes table for communication

  2. Security
    - Enable RLS on all tables
    - Create comprehensive policies for admin and user access
...
-- 3. Enhance shipments table with GPS tracking and driver relations
...
-- 13. Insert default admin user if not exists (optional)
...
-- 15. Update existing shipments to link with drivers (if needed)
-- This will attempt to match existing shipments with profiles based on driver name
-- Uncomment if you want to automatically link existing data:

/*
UPDATE shipments 
SET driver_id = p.id
FROM profiles p
WHERE shipments.supir = p.name 
